  ...
  const fetchDiscoverContent = async () => {
    try {
      ...
      const scholarships = allHubContent
        ...
        .map(content => ({
          img: content.featuredImage || content.thumbnailImage || "https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=800&q=80",
          title: content.title,
          time: `${content.amount || 'Amount varies'} • Deadline: ${content.deadline || 'Check details'}`,
      ...
      const competitions = allHubContent
        .filter(content => content.contentType === 'competition' || content.contentType === 'hackathon')
        .map(content => ({
          img: content.featuredImage || content.thumbnailImage || "https://images.unsplash.com/photo-1551434678-e076c223a692?auto=format&fit=crop&w=800&q=80",
          title: content.title,
          time: `${content.prize || 'Exciting prizes'} • ${content.category || 'Tech'}`,
      ...
      <div className="discover-page">
        ...
        <div className="discover-hero">
          <div className="discover-hero-content">
            ...
            <p>
              {filter === 'jobs' && 'Find the perfect job or internship opportunity that matches your skills and career goals'}
              {filter === 'events' && 'Join workshops, conferences, and events to enhance your skills and network'}
              {filter === 'scholarships' && 'Access funding and financial support for your educational journey'}
              {filter === 'competitions' && 'Participate in competitions and hackathons to showcase your talent'}
              {!filter && 'Explore jobs, internships, events, scholarships, and competitions all in one place'}
            </p>
            {filter && (
              <button 
      ...
      <div className="discover-content">
        {discoverData.length === 0 ? (
          <div className="discover-no-content">
            <h3>
              {filter === 'jobs' && 'No jobs or internships available at the moment'}
              {filter === 'events' && 'No events or workshops available at the moment'}
              {filter === 'scholarships' && 'No scholarships available at the moment'}
              {filter === 'competitions' && 'No competitions or hackathons available at the moment'}
              {!filter && 'No opportunities available at the moment'}
            </h3>
            <p>
              {filter ? 'Check back later for new opportunities in this category!' : 'Check back later for new jobs, internships, events, and more!'}
            </p>
            {filter && (
              <button 
                className="discover-view-all-btn"
          ...
          <div className="discover-event-slider-wrapper">
