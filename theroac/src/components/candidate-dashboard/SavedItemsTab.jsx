...
const SavedItemsTab = () => {
  ...
  const loadSavedItems = async () => {
    try {
      ...
      if (bookmarksResponse.status === "fulfilled") {
        ...
        const bookmarks =
          bookmarksResponse.value.bookmarks || bookmarksResponse.value || [];
      ...
      if (likesResponse.status === "fulfilled") {
        // Try different possible response formats
        const likes = likesResponse.value.likes || likesResponse.value || [];
        setLikedItems(likes);
      } else {
  ...
  const renderItemCard = (item, type) => {
    console.log("🔥 BACKEND ITEM EXACTLY:", item);
    const itemData = item.Job || item.Event || item.HubContent || item;
                ...
                <span>
                  {item.itemType === "internships" && itemData?.stipend
                    ? typeof itemData.stipend === "object"
                      ? itemData.stipend.amount
                        ? `₹ ${itemData.stipend.amount}/month`
                        : itemData.stipend.min && itemData.stipend.max
                          ? `₹ ${itemData.stipend.min} - ${itemData.stipend.max}/month`
                          : itemData.stipend.min
                            ? `₹ ${itemData.stipend.min}/month`
                            : "Competitive"
                      : itemData.stipend
                    : itemData?.salary
                      ? typeof itemData.salary === "object" &&
              ...
              <div className="meta-item">
                ...
                <span>
                  {item.itemType === "internships"
                    ...
                    : itemData?.jobType || itemData?.type || "Full-time"}{" "}
                  •{" "}
  ...
  const currentItems =
    activeSubTab === "bookmarks" ? bookmarkedItems : likedItems;
        ...
        {currentItems.length === 0 ? (
          ...
          <div className="saved-items-grid">
            {currentItems.map((item) =>
              renderItemCard(
                item,
                activeSubTab === "bookmarks" ? "bookmark" : "like",
              ),
            )}
          </div>
        )}
