...
import { X, Search, Download, Mail, FileText, MapPin, Calendar, User, Phone, Briefcase, ExternalLink } from 'lucide-react';
...
const JobEvaluationOverlay = ({ job, internship, onClose }) => {
  ...
  const filteredApplications = applications.filter(app => {
    ...
    if (selectedStage === 'all') {
      ...
      const hasSubmission = app.stageSubmissions && 
    ...
    if (selectedStatus === 'all') {
      ...
      matchesStatus = app.status === 'applied' || app.status === 'pending' || app.status === 'reviewing';
    } else if (selectedStatus === 'shortlisted') {
      matchesStatus = app.status === 'shortlisted' || app.status === 'interview';
  ...
  const getStageCount = (stageKey) => {
    ...
    if (stageKey === 'finalized') return applications.filter(app => app.status === 'hired' || app.status === 'offered').length;
  ...
  const getStatusCount = (status) => {
    ...
    if (status === 'in-progress') return applications.filter(app => app.status === 'applied' || app.status === 'pending' || app.status === 'reviewing').length;
    if (status === 'shortlisted') return applications.filter(app => app.status === 'shortlisted' || app.status === 'interview').length;
  ...
  return (
    <div className="job-eval-overlay">
        ...
        <div className="job-eval-header">
          <div className="job-eval-header-content">
            <div className="job-eval-header-left">
              {item.companyLogo && (
                <img src={item.companyLogo} alt={item.companyName} className="job-eval-company-logo" />
              )}
              <div>
                <h2>{item.title}</h2>
        ...
        <div className="job-eval-content-wrapper">
          ...
          <div className="job-eval-stages-section">
            <div className="job-eval-stages-header">
              ...
              <div className="job-eval-stage-actions">
                <button className="job-eval-btn-download" title="Download Profiles">
                  <Download size={16} />
                </button>
          ...
          <div className="job-eval-stages-list">
            ...
            >
              ...
              <span className="job-eval-stage-count">{getStageCount('all')}</span>
            </button>

            {/* Custom Stages (Assessment/Interview rounds) */}
            {hasCustomStages && customStages.map((stage, index) => {
              const stageIcon = stage.type === 'assessment' ? '📝' : 
                               stage.type === 'interview' ? '💼' : 
                               stage.type === 'final' ? '🎯' : '📊';
              const stageKey = `stage-${index}`;
              const stageLabel = stage.type === 'assessment' ? 'ASSESSMENT' :
                ...
                >
                  <div className="job-eval-stage-info">
                    ...
                    <div className="job-eval-stage-details">
                      ...
                      <span className="job-eval-stage-subtitle">{stage.title}</span>
                      {stage.deadline && (
                        <span className="job-eval-stage-deadline">
                          Due: {new Date(stage.deadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </span>
                      )}
                    </div>
                  </div>
                  <span className="job-eval-stage-count">{getStageCount(stageKey)}</span>
                </button>
              );
            })}
            ...
            >
              <div className="job-eval-stage-info">
                ...
                <div className="job-eval-stage-details">
                  ...
                  <span className="job-eval-stage-deadline">Hired/Offered</span>
                </div>
              </div>
              <span className="job-eval-stage-count">{getStageCount('finalized')}</span>
            </button>
          </div>
        </div>
          ...
          <div className="job-eval-status-tabs">
            {statusFilters.map(status => (
              ...
              >
                ...
                <span className="job-eval-tab-count">({getStatusCount(status)})</span>
              </button>
            ))}
          </div>
          ...
          {loading ? (
            <div className="job-eval-loading-state">Loading applications...</div>
              ...
              <table>
                <thead>
                  <tr>
                    <th>#</th>
                    <th>APPLICANT</th>
                    {selectedStage.startsWith('stage-') && <th>SUBMISSIONS</th>}
                    <th>REG. STATUS</th>
                    <th>ACTION / STATUS</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredApplications.map((app, index) => {
                    ...
                    const stageIndex = selectedStage.startsWith('stage-') 
                      ...
                      : null;
                    const stageSubmission = stageIndex !== null && app.stageSubmissions
                      ? app.stageSubmissions.find(sub => sub.stageIndex === stageIndex)
                      : null;
                    ...
                    <tr key={app.id}>
                        ...
                        <div className="job-eval-applicant-info">
                          <div className="job-eval-applicant-avatar">
                            {app.user?.profilePicture ? (
                              <img src={app.user.profilePicture} alt={app.user.fullName} />
                            ) : (
                          ...
                          <div className="job-eval-applicant-details">
                            <div className="job-eval-applicant-name">{app.user?.fullName || 'Unknown'}</div>
                            <div className="job-eval-applicant-email">{app.user?.email}</div>
                            {/* Resume Link */}
                            {(app.resumeUrl || app.resumeLink || app.user?.resume) && (
                              <a 
                                href={
                                  (app.resumeUrl || app.resumeLink || app.user?.resume)?.startsWith('http')
                                    ? (app.resumeUrl || app.resumeLink || app.user?.resume)
                            ...
                            {app.user?.city && (
                              <div className="job-eval-applicant-location">
                                <MapPin size={12} /> {app.user.city}, {app.user.country}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <td>
                        <div className="job-eval-status-badge-container">
                          ...
                          >
                            {app.status === 'hired' || app.status === 'selected' ? '✓ Complete' : 'Complete'}
                          </span>
                        </div>
                      </td>
                      <td>
                        <div className="job-eval-action-buttons">
                          ...
                          {(() => {
                            ...
                            const isFinalized = app.status === 'hired' || app.status === 'offered';
                            const isActioned = isRejected || isShortlisted || isFinalized;
                            ...
                            return (
                              <>
                                ...
                                <button 
                                  ...
                                  onClick={() => {
                                    if (selectedStage.startsWith('stage-')) {
                                      // Move to next stage
                                      const currentStageIndex = parseInt(selectedStage.split('-')[1]);
                                      const nextStageIndex = currentStageIndex + 1;
                                      const hasNextStage = item.stages && item.stages[nextStageIndex];
                                      
                                      if (hasNextStage) {
                                        // Move to next stage
                                        moveToNextStage(app.id, nextStageIndex);
                                      } else {
                                        // No more stages, mark as hired/offered
                                        updateApplicationStatus(app.id, 'offered');
                                      }
                                    } else {
                                      // Initial shortlist
                                  ...
                                  title={
                                    isActioned 
                                      ? 'Already actioned'
                                      : selectedStage.startsWith('stage-') 
                                        ? (item.stages && item.stages[parseInt(selectedStage.split('-')[1]) + 1]
                                ...
                                <button 
                                  ...
                                  disabled={isActioned}
                                  title={isActioned ? 'Already actioned' : 'Reject'}
                                >
                                  ✕
                                </button>
                                <button 
                                ...
                                <div className="job-eval-status-label">
                                  {isFinalized ? (
                                    ...
                                    <span className="status-shortlistedjob">✓ SHORTLISTED</span>
                                  ) : isRejected ? (
                                    <span className="status-rejected">✕ REJECTED</span>
                                  ) : selectedStage.startsWith('stage-') && item.stages ? (
                                    item.stages[parseInt(selectedStage.split('-')[1]) + 1]
      ...
      {selectedCandidate && (
        <div className="job-eval-candidate-modal-overlay" onClick={() => setSelectedCandidate(null)}>
          <div className="job-eval-candidate-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="job-eval-candidate-modal-body">
              <div className="job-eval-candidate-profile">
                {selectedCandidate.user?.profilePicture ? (
                  <img src={selectedCandidate.user.profilePicture} alt={selectedCandidate.user.fullName} />
                ) : (
              ...
              <div className="job-eval-candidate-info-grid">
                ...
                {selectedCandidate.user?.city && (
                  <div className="job-eval-info-item">
                    <MapPin size={16} />
                    <span>{selectedCandidate.user.city}, {selectedCandidate.user.country}</span>
                  </div>
                )}
                ...
                <div className="job-eval-info-item">
                  <Calendar size={16} />
                  <span>Applied: {new Date(selectedCandidate.createdAt).toLocaleDateString()}</span>
                </div>
              </div>
              ...
              {(selectedCandidate.resumeUrl || selectedCandidate.resumeLink || selectedCandidate.user?.resume) && (
                <a 
                  href={
                    (selectedCandidate.resumeUrl || selectedCandidate.resumeLink || selectedCandidate.user?.resume)?.startsWith('http')
                      ? (selectedCandidate.resumeUrl || selectedCandidate.resumeLink || selectedCandidate.user?.resume)
