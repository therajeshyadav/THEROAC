...
const EventParticipantsPage = () => {
  ...
  const isStageAccessible = (team, stageIndex) => {
    ...
    const previousStageEvaluation = team.evaluations?.find(
      (evaluation) => evaluation.stageIndex === stageIndex - 1,
    );
  ...
  const getStageStatus = (team, stage, stageIndex) => {
    ...
    const evaluation = team?.evaluations?.find(
      (evaluation) => evaluation.stageIndex === stageIndex,
    );
  ...
  const getStageDeadlineStatus = (stage) => {
    if (!stage?.deadline)
      return { status: "no-deadline", label: "No Deadline", color: "#757575" };
  ...
  const fetchEventParticipants = async () => {
    try {
      ...
      const eventParticipants =
        data.participants?.filter((p) => p.eventId === eventId) || [];
  ...
  const filteredParticipants = participants.filter((participant) => {
    const teamName =
      ...
      `Team ${participant.id?.toString().slice(-8)}` ||
      "Unnamed Team";
    ...
    if (filterStage !== "all") {
      ...
      if (filterStatus === "all") {
        ...
        matchesStage = ["submitted", "not-submitted"].includes(
          stageStatus.status,
        );
  ...
  const handleDownloadSubmission = async (submission) => {
    try {
      if (submission.fileUrl || submission.url || submission.value) {
        const fileUrl =
          submission.fileUrl || submission.url || submission.value;
  ...
  const handleContactTeam = (team) => {
    ...
    if (leaderEmail) {
      const subject = encodeURIComponent(
        `Regarding ${event?.title || "Event"} - Team ${team.teamName}`,
      );
      const body = encodeURIComponent(
        `Dear ${team.leader?.name || "Team Leader"},\n\nI hope this message finds you well.\n\nI am reaching out regarding your team's participation in ${event?.title || "the event"}.\n\nBest regards,\n[Your Name]`,
      );
      window.open(
        `mailto:${leaderEmail}?subject=${subject}&body=${body}`,
        "_blank",
      );
    } else {
  ...
  const handleStageEvaluation = async (
    teamId,
    stageIndex,
    status,
    feedback = "",
    ...
    try {
      const response = await fetch(
        `${API_URL}/events/${eventId}/teams/${teamId}/evaluate`,
        {
          method: "POST",
          headers: {
            ...
            {filteredParticipants.map((team) => (
              <div key={team.id} className="team-card">
                <div className="team-header">
                  <div className="team-info">
                    <h3 className="team-name">
                      {team.teamName ||
                        `Team ${team.id?.toString().slice(-8)}` ||
                        "Unnamed Team"}
                    </h3>
                    <div className="event-badge">
                      <div
                        className={`event-type type-${team.eventCategory || "other"}`}
                      >
                ...
                <div className="submission-statusEvent">
                  ...
                  <div className="stages-status compact">
                    {(event?.stages || []).map((stage, stageIndex) => {
                      ...
                      const key = `${team.id}-${stageIndex}`;
                      const isExpanded = expandedStage === key;

                      return (
                        <div
                          key={key}
                          className={`stage-row ${stageStatus.status}`}
                        >
                          {/* MAIN ROW */}
                          <div
                            className="stage-main"
                            onClick={() =>
                              setExpandedStage(isExpanded ? null : key)
                            }
                          >
                            <div className="stage-left">
                              ...
                              {stage.deadline && (
                                <span
                                  className={`stage-deadline ${isStageDeadlinePassed(stage) ? "expired" : ""}`}
                                >
                                  {new Date(
                                    stage.deadline,
                                  ).toLocaleDateString()}
                                  {isStageDeadlinePassed(stage) && " (Expired)"}
                                </span>
                              )}
                            </div>

                            <div
                              className={`stage-badge ${stageStatus.status}`}
                            >
                              {stageStatus.label}
                            </div>
                          </div>
                          ...
                          {/* EXPANDED CONTENT */}
                          {isExpanded && (
                            <div className="stage-expand">
                              {!isAccessible && (
                                <div className="stage-note locked">
                                  🔒 Complete previous stage to unlock
                                </div>
                              )}

                              {isAccessible &&
                                teamStage?.submissions?.length > 0 &&
                                teamStage.submissions.map((submission, i) => {
                                  console.log("FULL SUBMISSION OBJECT 👉", submission);
                                  return (
                                  <div key={i} className="submission-line">
                                    <span className="submission-label">
                                      {submission.label}
                                    </span>

                                    {/* FILES */}
                                    {submission.files?.map((file, j) => {
                                      console.log(
                                      ...
                                      return (
                                        ...
                                        >
                                          <span className="file-name">
                                            {file.url.split("_").slice(1).join("_")}
                                          </span>
                                          <button
                                            onClick={() =>
                                    ...
                                    {/* LINKS (GitHub etc) */}
                                    {submission.additionalFiles?.map(
                                      (link, k) => (
                                        <div
                                          key={k}
                                          className="submission-action"
              ...
              <div className="stages-evaluation">
                ...
                {(event?.stages || []).map((stage, stageIndex) => {
                  ...
                  const hasSubmissions =
                    evaluatingTeam.eventStages?.[stageIndex]?.submissions
                      ?.length > 0;
                  ...
                  return (
                    <div key={stageIndex} className="stage-evaluation">
                      <div className="stage-header">
                        ...
                        <span
                          className={`stage-status-badge ${stageStatus.status}`}
                        >
                          {stageStatus.label}
                        </span>
                      </div>
                      ...
                      {isAccessible &&
                        hasSubmissions &&
                        stageStatus.status !== "shortlisted" &&
                        stageStatus.status !== "rejected" && (
                          <div className="evaluation-actions">
                            <button
                              className="btn-shortlist"
                              onClick={() =>
                                handleStageEvaluation(
                                  evaluatingTeam.id,
                                  stageIndex,
                                  "shortlisted",
                                )
                      ...
                      {!isAccessible && (
                        <p className="stage-note">
                          Complete previous stage evaluation to unlock
                        </p>
                      )}
